...
## Adversarial critique amendments (applied)

A red-team pass reviewed the synthesized plan for fabrication risk, domain fit, buildability, and supervision framing. Its findings amend the plan as follows.

### Rescoped or removed features

- **MOI Question-Stage Quiz Generator Job** — As specified it composes questions from content 'already implied' by other tables, which is inference/authorship rather than retrieval — a direct zero-fabrication violation. Remove; if the manual is confirmed to contain literal Question-stage text, replace with a pure-retrieval lookup job instead.
- **Companion-Video Storyboard Generator Job + Media Contract** — The generative 'storyboard/shot-list' half asks a model to compose technique-cue wording for new instructional video content, risking unapproved physical-instruction guidance. Keep only the passive Media Contract (structured tagging schema for existing/incoming videos) and replace the generator with a human-facing template pre-filled with verbatim sourced fields only.
- **Squad Debrief Summarizer Job** — Open natural-language summarization of fault counts risks implying diagnoses or patterns never actually verified by a trainer, breaking the citation/provenance discipline applied everywhere else in the contract. Downgrade to Future until a strictly templated (non-generative) report format is defined.
- **Manual-Sourced Comprehension Quiz (Question Stage Digitized)** — As an MVP multiple-choice feature it cannot function without either inventing distractors or an unconfirmed assumption that the manual supplies them. Demote from MVP and rescope to verbatim recall/matching format, contingent on the proposed Assessment-Item Constraint Policy.
- **Stage-Gated Mastery Certification** — Auto-computing a 'certified' status from quiz results plus self/buddy-checked reps, with no required live-trainer confirmation, conflicts with the mandatory supervision framing for physical movement correction. Replace with a 'Trainer-Confirmed Readiness Status' gated by the proposed Trainer Sign-Off feature.

### Added features (all MVP)

- **Supervision Reminder & Scope-of-Use Banner** (Trainee App)  
  A persistent, unobtrusive statement shown at first launch and on every self-practice screen (checklists, cadence trainer, quiz) stating that these tools support personal revision and reference, and that correct execution and any readiness/certification claim must be verified by a qualified trainer in person.  
  *Why:* This is the direct product expression of the required 'never implies it replaces a live trainer for physical correction' constraint, which the current plan has no feature enforcing anywhere.
- **Trainer Sign-Off / Verified-Observation Gate** (Trainer App)  
  A single, fast trainer action during live Practice ('observed — meets standard') that is the only way a trainee's mastery status can advance past 'self-reported'; unsupervised checklist ticks and quiz scores alone can never move a trainee into a cleared/ready state.  
  *Why:* Makes supervision structurally required rather than aspirational — it is the concrete mechanism that keeps any readiness or certification claim tied to an actual human observing the physical movement.
- **Observation-Source Tagging on All Assessment Data** (Shared Content Platform & Data)  
  Every fault tag, checklist tick, and mastery-state change carries a required source field (trainer-observed / trainee-self-reported / buddy-checked), surfaced wherever the data is shown — ledger, dashboard, or copilot query results.  
  *Why:* Without this, the Squad Readiness Dashboard and mastery ledger cannot be trusted for real operational decisions, since supervised and unsupervised signals are currently indistinguishable once merged.
- **Figure/Media Asset Intake Tool** (Shared Content Platform & Data)  
  A simple upload-and-tag interface for a content admin to attach an approved photo, ghost-progression diagram, or video file directly to a specific (drill, stage, figure-slot) pointer, distinct from the docx-text extraction pipeline, gated by the same per-field approval step before it can render live.  
  *Why:* Photos, diagrams, and videos are exactly the content type the docx extraction pipeline cannot pull automatically (they are literally '[Insert image]' placeholders) — without a dedicated intake path there is no way to actually close the pilot's most common gap.
- **Drill Coverage Status at a Glance** (Trainer App)  
  A single view, shown before session planning and in the roster, listing each of the ~10 drills as fully authorized / partially authorized / pending, so a trainer choosing drills for a session (and a trainee browsing) knows upfront which drills are actually usable rather than discovering gaps mid-session.  
  *Why:* Given this chapter has drills with entirely empty tables, surfacing coverage proactively prevents wasted session-planning effort and sets honest, sized expectations for what the pilot actually covers.
- **Manual-Sourced Assessment-Item Constraint Policy** (Shared Content Platform & Data)  
  A hard authoring rule defining the only permitted ways to build a quiz item without invention: recall prompts against verbatim fields, true/false against verbatim statements, or SME-pre-authored multiple-choice items entered and approved as their own content type with their own provenance record — never model-composed distractors or questions.  
  *Why:* This closes the specific mechanism by which the quiz features could otherwise slide into fabrication, by making 'where did this wrong answer come from' an answerable, auditable question for every quiz item in the system.
- **Generation-Boundary Manifest** (Trainer Copilot Contract)  
  A single machine-readable list shipped with the frozen contract enumerating job types as pure-retrieval (safe: Q&A, glossary lookup, fault-checklist assembly), composition-adjacent (requires mandatory human review before use), or explicitly forbidden (must never be built against this contract: new technique cues, new commands, new question wording).  
  *Why:* Since this project only defines the data contract and does not build the copilot's reasoning, this manifest is the only real lever this team has to stop a future integrator from building the exact fabrication-risk jobs identified in this review — without it, zero-fabrication is a stated intent, not an enforceable spec.
- **Minimum Viable Access Gate (interim auth placeholder)** (Technical & Mobile Platform)  
  A lightweight passcode- or invite-link-based access control for the pilot (not full RBAC) that keeps content off the open web while the classification/security review question remains unresolved, upgradeable later to full role-based access.  
  *Why:* Running a completely open pilot while the plan's own open questions admit the data classification is unknown is an avoidable exposure for military training content, even a nominally sanitized chapter.

### Standing findings

- **[HIGH]** MCQ quiz feature cannot work as specified without inventing content — 'Manual-Sourced Comprehension Quiz (Question Stage Digitized)' is MVP and explicitly promises 'multiple-choice ... with no generated distractors,' but a multiple-choice question requires wrong answers from somewhere. The manual almost certainly does not supply pre-written incorrect options. As specified, the only ways to build this are either (a) invent distractors — direct fabrication of drill content — or (b) the feature is not actually buildable in MCQ form. This is a hidden fabrication risk sitting inside an MVP item.
- **[HIGH]** Copilot 'quiz generator' job composes questions from 'implied' content — The job description says it assembles questions 'using only content already implied by that drill's Word-of-Command and Stages tables.' Content that is 'implied' but not literally present is something a model must infer and phrase — that is authorship, not retrieval, and directly violates the zero-fabrication constraint the rest of the contract is built to enforce.
- **[HIGH]** Video storyboard generator invents technique-cue language for new instructional media — Turning a manual's 'Common Faults' bullets (phrased as errors) into positively-phrased on-screen technique cues (as seen in the one real video, e.g. 'Lift thigh parallel to ground') is a rewording/inference step, not verbatim extraction. For a precision physical-execution skill, an AI-composed shot list risks encoding unapproved technique guidance into a video that will be shown as authoritative demonstration.
- **[MEDIUM]** Debrief summarizer risks generating diagnosis/pattern language beyond logged data — 'Produces a plain-language squad debrief' from fault counts is open natural-language generation; without a strict template it can imply causal patterns or severity judgments a trainer never actually made, undermining the audit-everything discipline applied everywhere else in the contract.
- **[MEDIUM]** Generative jobs are scheduled before their containment mechanism — Three composition-adjacent jobs (Quiz Generator, Storyboard Generator, Debrief Summarizer) are Phase2, while the 'Human-Approved Content Suggestion Loop' — the mechanism meant to catch and formally vet copilot-composed output before it reaches the authoritative corpus — is deferred to Future. This inverts the intended risk order: don't ship generation before its safety valve.
- **[HIGH]** No feature anywhere states the app supports but does not replace live trainer correction — The plan has no disclaimer/framing feature enforcing this explicit hard requirement. Combined with a solo/buddy 'Common Faults Self-Check Checklist' and an auto-computed 'Stage-Gated Mastery Certification,' a trainee could reach an app-reported 'certified' state without a qualified instructor ever observing them execute the drill.
- **[MEDIUM]** Mastery ledger and readiness dashboard don't distinguish trainer-observed data from unsupervised self-report — Fault tags from live trainer observation and trainee self-checklist ticks/self-confidence tags both feed the same 'mastery ledger' and roll up into the Squad Readiness Dashboard with no source field distinguishing them. A trainer reading '95% faults cleared' can't tell if that means 95% were watched by an instructor or 95% self-ticked a box — a false-confidence risk for an operational readiness decision.
- **[LOW]** 'Flashcard'/self-confidence-tag framing edges toward consumer app tone — 'Self-Check Glossary & Command-Syllable Recall Drills' (flashcard-style) and 'Not Started/Practicing/Confident' self-tags sit close to the consumer-gamified language the brief explicitly asks to avoid in this formal/functional discipline-training context, even though no points/streaks/badges are present.
- **[HIGH]** MVP scope is far too large to validate a single-chapter pilot — Roughly 49 features are tagged MVP across six surfaces, including a full 9-item Trainer Copilot Contract (for a copilot no one on this team is even building yet), a complete mastery ledger + readiness dashboard, live multi-part session-runner tooling, and the entire ingestion/governance pipeline (extraction, placeholder tagging, per-field state machine, SME console, provenance audit view, no-fabrication firewall) — all before a single trainer has run one real session. This is not a minimal validation slice.
- **[MEDIUM]** Deterministic no-LLM docx extraction may be harder than treated — The pipeline promises fully rule-based table-cell parsing with 'no LLM step anywhere,' but the source is an actively-edited draft with inconsistent formatting, merged cells, and mixed placeholder/real content. The plan's fallback (mandatory manual field-mapping for ambiguous tables) is the right instinct but likely underestimates the actual manual-curation effort needed for a single pilot chapter.
- **[MEDIUM]** Change-audit logging deferred past the period it's most needed — 'Immutable Content Edit Audit Log' and 'Manual Revision Diff & Re-ingestion Workflow' are both Phase2, yet the plan's own MVP narrative assumes active SME correction of this changing draft happens during the pilot window. Corrections made during MVP with no append-only trail undermine the 'prove every fact traces to an authorized passage' compliance goal the plan itself sets as the success bar.
- **[MEDIUM]** Most MVP live-session features will only be exercisable on a minority of the ~10 drills — Live Fault Tagging, the Stage-by-Stage Demonstration Deck, Companion Video Playback, and the Cadence Reference all depend on content that is confirmed absent for at least 4 of 10 drills (Salutation, Mark Time, Three Cheers, Pledge/Creed have empty tables) and on video assets that exist for only one drill total. The pending-state banner handles this honestly at the UI level, but the plan never states the resulting real content coverage at launch, risking a pilot scoped against an assumed 10-drill catalog that doesn't actually exist yet.
- **[HIGH]** Quiz features assume the manual contains literal, ready-to-use question text — The observed schema only confirms the MOI table has a 'Question' stage whose answers 'point back to' other tables — it does not confirm the manual writes out full question sentences per drill. If Question-stage rows are templated/sparse rather than literal questions, both quiz features have no real verbatim source to draw from and cannot be built as specified without generation. This should be verified against actual source content before either quiz feature stays at its stated priority.
- **[MEDIUM]** No stated boundary on how ghost-progression diagrams get created — The plan tracks ghost-progression diagrams as an asset type in the Figure Reference registry but never states whether they are human-designed composites of real approved photos or could be produced by some automated/generative image process. Since zero-fabrication applies to visual technique demonstration as much as text, this boundary needs to be explicit, not assumed.
- **[MEDIUM]** No tool for actually attaching photos/diagrams/videos to pending slots — The SME Review Console approves/rejects/flags extracted text fields, and the Media Fulfillment Registry tracks slot status, but nothing describes how a human uploads a new photo or video to fulfill an '[Insert image]' placeholder. Since these assets are by definition not docx-extractable, this is a real gap in the path to actually resolving the pilot's most common content gap.
- **[MEDIUM]** 'Certification' language implies formal qualification without instructor sign-off — 'Stage-Gated Mastery Certification' marks a trainee/squad 'certified' purely from quiz results plus fault-checklist reps, with no requirement that a live trainer ever confirmed the physical execution. In a military discipline-training context, 'certified' status issued by an app rather than an instructor is a supervision-framing problem, not just a naming quibble.
- **[LOW]** MVP pilot appears to ship with no access control while classification is unresolved — 'Role-Based Access & Unit-Scoped Provisioning' is Phase2, meaning the MVP pilot is implicitly open/unauthenticated, while the plan's own open questions admit the security/data classification of even this 'sanitized' chapter is unknown. Running a fully open pilot before that question is answered is an avoidable exposure for military training content.

### Pilot-slice consequence

The 49-feature MVP set is a *program* target, not a first-build target. The first build ("pilot slice") is: deterministic ingestion of Chapter 2 Section 1 with per-field pending tagging -> trainee roster/detail/glossary with honest coverage states and the supervision banner -> trainer session runner (layout/ratio lookup, MOI stepper, fault chips, Semula counter) -> static copilot contract export with the generation-boundary manifest. Quizzes ship as verbatim recall only. Everything else follows once one real squad session validates the slice.